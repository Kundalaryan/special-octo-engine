...
import { StatusBadge, formatEnum } from '../components/StatusBadge';
...
const getStatusBtnColor = (status: IssueStatus) => {
  switch (status) {
    ...
    case 'REJECTED': return 'text-gray-600 bg-gray-50 border-gray-100';
...
const IssueCard = ({ issue }: { issue: Issue }) => {
  ...
  return (
    <div className="bg-white rounded-xl p-5 border border-gray-200 shadow-sm hover:shadow-md transition-all flex flex-col h-full relative group">
        
        {/* Header with StatusBadge */}
        <div className="flex justify-between items-start mb-3">
            <span className="text-xs font-mono text-gray-400">TICKET #{issue.id}</span>
            <StatusBadge className={getSeverityColor(issue.severity)}>
                {issue.severity}
            </StatusBadge>
        </div>

        <h3 className="font-bold text-gray-900 text-lg mb-1 leading-tight">{formatEnum(issue.issueType)}</h3>
        ...
        <p className="text-sm text-gray-500 line-clamp-3 mb-6 flex-grow">"{issue.description}"</p>
        ...
        <div className="pt-4 border-t border-gray-100 flex items-center justify-between mt-auto relative">
            <div className="relative">
                <button 
                    ...
                    disabled={isLoading}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-bold uppercase transition-colors 
                    ${getStatusBtnColor(issue.status)} ${isLoading ? 'opacity-50' : 'hover:brightness-95'}`}
                >
                ...
                {isDropdownOpen && (
                    <>
                        ...
                        <div className="absolute left-0 bottom-full mb-2 w-48 bg-white border border-gray-100 rounded-xl shadow-xl overflow-hidden z-20 animate-fade-in">
                            <div className="p-1">
                                {issue.status === 'OPEN' && (
                                    <button onClick={() => acknowledgeMutation.mutate()} className="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg flex items-center gap-2">
                                        <PlayCircle size={14} /> Mark In Progress
                                    </button>
                                )}
                                {issue.status !== 'RESOLVED' && (
                                    <button onClick={() => resolveMutation.mutate()} className="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 rounded-lg flex items-center gap-2">
            ...
            <span className="text-xs text-gray-400">{formatDistanceToNow(parseISO(issue.createdAt), { addSuffix: true })}</span>
        </div>

        <div className="absolute bottom-4 right-28 opacity-0 group-hover:opacity-100 transition-opacity">
...
const Support = () => {
  ...
  const { data: response, isLoading, isFetching } = useQuery({
    ...
    queryFn: async () => {
      const params: any = { page, size: ITEMS_PER_PAGE };
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto font-sans">
      ...
      {/* Grid Content */}
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-pulse">{[1,2,3,4].map(i => <div key={i} className="h-64 bg-gray-200 rounded-xl"></div>)}</div>
