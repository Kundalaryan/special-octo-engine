...
const Feedback = () => {
  ...
  const rawSuggestions = useMemo(() => response?.data?.content || [], [response]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {!isLoading && displayedSuggestions.length === 0 && (
          <div className="col-span-full text-center py-20 bg-white rounded-xl border border-gray-200 border-dashed">
            <div className="inline-flex justify-center items-center w-16 h-16 rounded-full bg-gray-50 mb-4"><MessageSquare className="text-gray-400" size={24} /></div>
        ...
        {displayedSuggestions.map((item) => (
          <div key={item.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 flex flex-col h-full">
